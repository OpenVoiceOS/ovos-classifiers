...
from ovos_classifiers.skovos.nn import PerceptronTrainer, MLPTrainer

csv_path = f"{dirname(__file__)}/datasets/ocp_playback_type_v0.csv"
ents_csv_path = f"{dirname(__file__)}/datasets/ocp_entities_v0.csv"

clf_path = f"{dirname(__file__)}/classifiers/media_ocp/perceptron/cv2_lemma_1704443382.clf"
...
|----------|----------|----------|--------|----------|"""
    ...
    for i in range(n_searches):
        ...
        n = f"cv2_lemma_{str(training_run.end_ts).split('.')[0]}"
...
def find_best_hyperparams_mlp(n_searches=5):
    mkdown_table = """
| pipeline | language | accuracy | params | size (MB)|
|----------|----------|----------|--------|----------|"""
    ...
    for i in range(n_searches):
        trainer = MLPTrainer("raw", featurizer)
        training_run = trainer.hyperparam_search(csv_path)
        n = f"cv2_lemma_{str(training_run.end_ts).split('.')[0]}"
        ...
        with open(f"{dirname(__file__)}/reports/{clf_id}/mlp/bias_hyperparam_report.md", "w") as f:
            f.write(mkdown_table)
